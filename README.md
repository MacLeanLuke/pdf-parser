...
Eligibility PDF Ingestor is a small, production-ready Next.js 14 application that helps housing and homeless-services teams turn program PDFs into structured eligibility data. Upload a single PDF, let the app extract text, validate it with OpenAI via the Vercel AI SDK, and store the structured result in PostgreSQL using Drizzle ORM.

## Features

- Upload a single PDF from the homepage and trigger server-side parsing with `pdf-parse`.
...
- Browse recent ingestions from the built-in history list.
...
- `app/` – App Router pages and API routes (`/api/parse-eligibility`, `/api/history`).
